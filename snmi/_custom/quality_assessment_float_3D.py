...
class QualityAssessment:
    ...
    def assessment(self, dataset, 
        ...
        for i in tqdm.tqdm(range(ndata)):
            ...
            for mi, r in enumerate(results):
                ...
                lab_nii = nib.Nifti1Image(r[1], affine)
                nib.save(lab_nii, tmp_lab_path)
                # weight_nii = nib.Nifti1Image(r[1], affine)
                # nib.save(lab_nii, tmp_lab_path)
    ...
    def compare(self, data_dict, n_classes, combine_method, target_key):
        ...
        for i in range(len(source_dicts)-1):
            random_aug = RandomAug(org.shape[2:])
    ...
    def __init__(self, shape):
        methods = [Rotate(np.random.randint(-20, 20)), 
                #    Flip(np.random.choice([2,3])), 
                   Shift(np.random.randint(-shape[0]/10, shape[0]/10), np.random.randint(-shape[1]/10, shape[1]/10), np.random.randint(-shape[2]/10, shape[2]/10))]
...
class Shift:
    ...
    def aug(self, img):
        ...
        img_ = ndimage.shift(img, (0, 0, self.shift_x, self.shift_y, self.shift_z), mode='constant', cval=0)
        return img_
    def reverse(self, img):
        img_ = np.zeros_like(img)
        img_[:,0,...] = ndimage.shift(img[:,0,...], (0, -self.shift_x, -self.shift_y, -self.shift_z), mode='constant', cval=1)
        img_[:,1:,...] = ndimage.shift(img[:,1:,...], (0, 0, -self.shift_x, -self.shift_y, -self.shift_z), mode='constant', cval=0)
        return img_
