...
from snmi.nets.net_reg_3D import MrReg
from snmi.nets.unet3D import Unet3d
from snmi._custom.quality_assessment_float_3D import QualityAssessment
from models import JointSegModel, JointRegModel

import sys

# Set up paths for dataset
PATH_TO_3D_DATASET = "PATH_TO_3D_DATASET"  # Replace with actual path to 3D dataset (e.g., OASIS)


class cfg:
    data_path = PATH_TO_3D_DATASET  # 3D MRI dataset path
    output_path = './results_3D'
    ...
    pre_disp_loss = {LF.Grad('l2'): [16, 8, 4, 2]}
    ...
    reg_loss = {LF.GNCC(): [1, 1, 1, 1]} 
    disp_loss = {LF.Grad('l2'): [16,8,4,2]}
...
# Network
net_seg = Unet3d(n_classes=5, in_channels=1, n_layers=cfg.seg_layers)
...
ass_pre = {cfg.img_suffix: [P.min_max, P.ExpandDim(0), P.ToTensor(0)],
            cfg.lab_suffix: [P.OneHot([0,1,2,3,4])]}
...
if not cfg.test_only and cfg.iters_start == 0:
