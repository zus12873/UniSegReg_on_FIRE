    ...
    for i in range(x.shape[0]):
        if x.ndim == 5:
            ...
            for d in range(x.shape[-1]):
                s = structural_similarity(x[i,...,d].squeeze(), y[i,...,d].squeeze(), data_range=x.max() - x.min())
                ssim_vals.append(s)
            res.append(np.mean(ssim_vals))
        else:
            s = structural_similarity(x[i].squeeze(), y[i].squeeze(), data_range=x.max() - x.min())
            res.append(s)
    return np.array(res)



def hausdorff(x, y, ignore_background=False):
    res = compute_hausdorff_distance(y, x, include_background=not ignore_background)
    return res
