...
class Encoder(nn.Module):
    def __init__(self, in_channel, n_channel, n_layers=5):
        ...
        for i in range(n_layers-1):
            dw = nn.Conv3d(n_channel, n_channel, 3, 2, padding=1, bias=True)
...
class DecLayer(nn.Module):
    def __init__(self, n_channel, int_step, up=True):
        ...
        self.deconv = None
        if up:
            self.deconv = nn.ConvTranspose3d(n_channel, n_channel, 2, 2, padding=0, bias=True)
            self.conv = nn.Conv3d(n_channel*2, n_channel, 3, padding='same', bias=True)
        else:
            self.conv = nn.Conv3d(n_channel, n_channel, 3, padding='same', bias=True)
        self.flow_pos = nn.Conv3d(n_channel, 3, 3, padding='same', bias=True)
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channel, out_channel):
        super().__init__()
        self.conv1 = nn.Conv3d(in_channel, out_channel, 3, padding='same', bias=True)
        self.conv2 = nn.Conv3d(out_channel, out_channel, 3, padding='same', bias=True)
        self.addition = nn.Conv3d(in_channel+out_channel, out_channel, 3, padding='same', bias=True)
