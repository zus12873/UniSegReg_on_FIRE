...
from snmi.nets.layers import SpatialTransformer, ResizeTransform
from snmi.models.regis_tools import *


class JointRegModel(BaseModel):
...
# for k, i in enumerate([16,32,64,128,256]):
    
#     cv2.imwrite(f'./_test_fig/res_vis/source_{i}.jpg', cv2.resize(F.interpolate(source_img, size=[i,i], mode='nearest').cpu().numpy()[1][0]*255, [256, 256], interpolation=cv2.INTER_NEAREST))
#     cv2.imwrite(f'./_test_fig/res_vis/target_{i}.jpg', cv2.resize(F.interpolate(target_img, size=[i,i], mode='nearest').cpu().numpy()[1][0]*255, [256, 256], interpolation=cv2.INTER_NEAREST))
#     isource = F.interpolate(source_img, size=[i,i], mode='nearest')
#     iwrapped = SpatialTransformer(size=[i,i])(isource, disps[k][0])
#     cv2.imwrite(f'./_test_fig/res_vis/14-1/warp_{i}.jpg', cv2.resize(iwrapped.cpu().numpy()[1][0]*255, [256, 256], interpolation=cv2.INTER_NEAREST))

#     iwrapped = SpatialTransformer(size=[256,256])(source_img, F.interpolate(disps[k][0], size=[256,256], mode='nearest') * 2**(4-k))
#     cv2.imwrite(f'./_test_fig/res_vis/14-1/warp_resize{i}.jpg', cv2.resize(iwrapped.cpu().numpy()[1][0]*255, [256, 256], interpolation=cv2.INTER_NEAREST))
    ...
    def eval_step(self, batch, **kwargs):
        ...
        ssim = EF.ssim(target_img.cpu().numpy(), wrapped_img.cpu().numpy())
        ssim_baseline = EF.ssim(target_img.cpu().numpy(), source_img.cpu().numpy())
        ...
        eval_results = {'loss': loss,
                        ...
                        'ssim': ssim.mean(),
                        'ssim_baseline': ssim_baseline.mean(),
                        }
        # if np.isnan(loss):
        #     print('non')
        if self.source_lab_suffix is not None:
            ...
            hausdorff_baseline = EF.hausdorff(source_lab, target_lab, ignore_background=True).cpu().numpy()
            hausdorff = EF.hausdorff(wrapped_lab, target_lab, ignore_background=True).cpu().numpy()
            eval_results.update({'dice': dice, 'dice_baseline': dice_baseline,
                                'hausdorff': hausdorff, 'hausdorff_baseline': hausdorff_baseline})
